...
export function buildDispatchMessage(params: {
  ...
  if (params.isFirstCall) {
    ...
    parts.push(
      ...
      `7. 单纯确认已读、确认收到、礼貌回应时，不要 @任何人。`,
      `8. 如果你的回复已经是最终结果，不需要任何人继续处理，则 footer 控制块里不要写 @角色名。`,
      `9. 对话务必简洁扼要，不要寒暄，不要客套，不要重复上下文。`,
    ...
    if (isLeader) {
      parts.push(
        ...
        `10. 如果你只是回答一个小问题、闲聊、补充说明，且不代表群任务进入或继续推进，就不要输出任何状态标记。`,
        ``,
        `[组长处理原则]`,
