...
- 只负责“群创建 / 删除 / 成员管理 / leader 管理 / 查群状态 / 发群消息”
...
- 它是一份简洁的进度摘要，不是聊天记录副本
- 内容通常包括：当前进展、关键事项、阻塞项
- 条目状态支持：`待开始 / 进行中 / 已完成 / 阻塞`

这条能力底层走的是隐藏的 `manage_group_plan` tool，而不是正文输出协议。

现在的使用方式是：

- leader 在协作推进过程中更新 Plan
- 其他 agent 也可以在需要时读取 Plan
- 用户在群聊头部 `...` 菜单里点击 `查看 Plan`
- 弹窗里可以直接看到摘要、条目、最近更新时间和更新者标签
...
6. Plan 是摘要，不是聊天记录
如果你想看完整推理和上下文，还是要看群消息本身；Plan 更适合快速查看进度。
