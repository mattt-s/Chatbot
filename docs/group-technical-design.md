...
3. leader 在回复末尾单独输出对应标记，app 负责解析并更新状态：
...
leader 的首轮注入提示里会明确说明各状态标记的适用场景，以及不输出任何标记的情形（小问题、闲聊、补充说明等）。

此外，用户也可以在群聊头部手动切换到任意状态。手动操作只修改”当前状态值”，不会屏蔽 leader 的后续控制；leader 之后再次输出状态标记时，仍然会继续覆盖群状态。
...
当前群成员之间的核心信息（文件路径、当前进度、当前阻塞点等）只能靠群内消息传递，缺乏一个结构化的低成本共享机制。

后续可引入”群共享记忆板”概念：

- 群内每个成员维护自己的一块记忆区（key-value 或简短条目），只记录自己相关的核心信息
- 所有成员的记忆区汇总后，通过注入提示词的方式在每次 dispatch 前透传给对应角色
- 成员可以读取所有人的记忆，但只写自己的那一块
- 记忆内容要求精简，仅记录核心信息：如负责的文件路径、当前任务进度、当前遇到的问题等
- 目标是让群内消息本身可以更简洁，减少成员之间大段重复背景说明，核心上下文通过记忆板获取即可

这条机制可以通过新增一个 `manage_group_memory` tool 实现，职责包括：读取当前所有成员记忆、更新自己的记忆条目、清空自己的记忆。

### 群成员注入词补充：文档内容输出到项目目录

当前群成员有时会把大段文档内容直接输出到聊天框，导致群消息冗长、可读性差。

后续在群成员首轮注入提示词里明确约定：

- 文档类内容（代码文件、设计文档、配置文件、README 等）应直接写入项目目录下对应路径
- 聊天框只用于简短说明、进度汇报、问题讨论等结构性沟通
- 避免在群消息里输出大段代码块或长篇文档正文

### 群成员注入词定期重注入

群成员的角色提示词仅在首轮注入一次，随着对话轮次增加，角色可能逐渐漂移、不再遵守注入规则。

后续实现策略：

- 首轮 dispatch 时正常注入一次完整提示词
- 之后每隔 10 次回复，自动重新注入一次完整提示词（或关键规则摘要）
- 注入时机：在 dispatch 前拼入 system prompt 或作为隐式 user 消息注入，具体方式视 provider 接口决定
- 目标是长期协作场景下保持角色持续遵守规则，避免”跑偏”问题
